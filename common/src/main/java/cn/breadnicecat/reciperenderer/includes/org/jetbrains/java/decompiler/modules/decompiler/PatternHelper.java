...
public final class PatternHelper {
	...
	public static void replaceAssignmentsWithPatternVariables(@NotNull RootStatement statement, @NotNull StructClass structClass) {
		...
		List<SwitchHelper.TempVarAssignmentItem> tempVarAssignments = new ArrayList<>();
	...
	private static PatternVariableCandidate findSimpleCandidateFromIfStat(@NotNull Statement ifElseStat,
		...
		if (ifElseExprents == null || ifElseExprents.isEmpty() || ifElseExprents.get(0).type != Exprent.EXPRENT_ASSIGNMENT)
			return null;
	...
	private static PatternVariableCandidate findRecordPatternCandidate(@NotNull Statement parent,
		...
		int nextIndex = i;
		List<SwitchHelper.TempVarAssignmentItem> tempVarAssignmentItems = new ArrayList<>();
	...
	private static PatternVariableCandidate findRecursivelyInstanceOfIfStatement(@NotNull Statement statement,
	                                                                             ...
	                                                                             @NotNull List<SwitchHelper.TempVarAssignmentItem> tempVarAssignmentItems) {
	...
	private static boolean processCatchStatement(@NotNull VarTracker tracker,
		...
		if (!((throwExpected instanceof ExitExprent exitExprent && exitExprent.getExitType() == ExitExprent.EXIT_THROW &&
	...
	static class PatternVariableCandidate {
		...
		private final @NotNull List<SwitchHelper.TempVarAssignmentItem> tempVarAssignments;
	...
	static class VarTracker {
		...
		@NotNull
		private final List<SwitchHelper.TempVarAssignmentItem> varTempAssignmentTracker = new ArrayList<>();
		...
		void put(@NotNull VarExprent varExprent,
			...
			if (statement != null) {
				varTempAssignmentTracker.add(new SwitchHelper.TempVarAssignmentItem(varExprent, statement));
			}
		}
		...
		void addPreserve(@NotNull VarExprent varExprent, @NotNull Statement statement) {
			varTempAssignmentTracker.add(new SwitchHelper.TempVarAssignmentItem(varExprent, statement, false));
		}
