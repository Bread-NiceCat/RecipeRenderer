...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.decompose.FastExtendedPostdominanceHelper;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.deobfuscator.IrreducibleCFGDeobfuscator;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.stats.*;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.FastFixedSetFactory;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.FastFixedSetFactory.FastFixedSet;
...
public final class DomHelper {
			...
			while (true) {
				...
				for (int i = 0; i < lst.size() - 1; i++) {
					...
					if (current.isMonitorEnter()) {
						...
						while (next.type == Statement.StatementType.SEQUENCE) {
							next = next.getFirst();
						}
						
						if (next.type == Statement.StatementType.CATCH_ALL) {
	...
	private static boolean processStatement(Statement general, HashMap<Integer, Set<Integer>> mapExtPost) {
		...
		if (general.type == Statement.StatementType.ROOT) {
