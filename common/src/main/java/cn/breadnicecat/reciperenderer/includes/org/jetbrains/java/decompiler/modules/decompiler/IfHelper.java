...
public final class IfHelper {
  ...
  public static boolean mergeIfs(Statement statement, Set<? super Integer> setReorderedIfs) {
    if (statement.type != Statement.StatementType.IF && statement.type != Statement.StatementType.SEQUENCE) {
      return false;
    }
  ...
  private static IfNode buildGraph(IfStatement stat, boolean stsingle) {
    ...
    if (ifchild == null) {
      ...
      if (ifchild.type == Statement.StatementType.IF && ((IfStatement)ifchild).iftype == IfStatement.IFTYPE_IF) {
    ...
    if (stsingle || edge.getType() != StatEdge.EdgeType.REGULAR) {
      ...
      if (elsechild.type == Statement.StatementType.IF && ((IfStatement)elsechild).iftype == IfStatement.IFTYPE_IF) {
  ...
  private static boolean reorderIf(IfStatement ifstat) {
    ...
    Statement from = parent.type == Statement.StatementType.SEQUENCE ? parent : ifstat;
    ...
    Statement last = parent.type == Statement.StatementType.SEQUENCE ? parent.getStats().getLast() : ifstat;
