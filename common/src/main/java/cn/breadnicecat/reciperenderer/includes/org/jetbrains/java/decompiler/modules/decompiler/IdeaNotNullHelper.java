...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.attr.StructAnnotationAttribute;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.attr.StructAnnotationParameterAttribute;
...
public final class IdeaNotNullHelper {
	...
	private static boolean findAndRemoveParameterCheck(Statement stat, StructMethod mt) {
		...
		while (st.type == Statement.StatementType.SEQUENCE) {
			st = st.getFirst();
		}
		
		if (st.type == Statement.StatementType.IF) {
			...
			if (ifbranch != null &&
					...
					ifbranch.type == Statement.StatementType.BASIC_BLOCK &&
	...
	private static boolean removeReturnCheck(Statement stat, StructMethod mt) {
		...
		if (parent != null && parent.type == Statement.StatementType.IF && stat.type == Statement.StatementType.BASIC_BLOCK && stat.getExprents().size() == 1) {
			...
			if (exprent.type == Exprent.EXPRENT_EXIT) {
					...
					if (ifparent.getElsestat() == stat && if_condition.type == Exprent.EXPRENT_FUNCTION &&
						...
						if (second_param.type == Exprent.EXPRENT_CONST &&
							...
							if (first_param.equals(exprent_value)) {        // TODO: check for absence of side effects like method invocations etc.
								if (ifbranch.type == Statement.StatementType.BASIC_BLOCK &&
										ifbranch.getExprents().size() == 1 &&
				...
				parent.type == Statement.StatementType.SEQUENCE &&
				stat.type == Statement.StatementType.BASIC_BLOCK &&
			...
			if (exprent.type == Exprent.EXPRENT_EXIT) {
				...
				if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
					...
					if (sequence_stats_number > 1 &&
							...
							sequence.getStats().get(sequence_stats_number - 2).type == Statement.StatementType.IF) {
