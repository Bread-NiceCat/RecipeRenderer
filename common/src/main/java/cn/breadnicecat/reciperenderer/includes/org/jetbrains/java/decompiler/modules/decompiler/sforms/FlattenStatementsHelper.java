...
public class FlattenStatementsHelper {
	...
	public DirectGraph buildDirectGraph(RootStatement root) {
		...
		DirectNode node = new DirectNode(DirectNode.DirectNodeType.DIRECT, dummyexit, Integer.toString(dummyexit.id));
	...
	private void flattenStatement() {
		...
		while (!lstStackStatements.isEmpty()) {
			...
			if (statEntry.succEdges == null) {
				...
				switch (stat.type) {
					case BASIC_BLOCK -> {
						node = new DirectNode(DirectNode.DirectNodeType.DIRECT, stat, (BasicBlockStatement) stat);
						...
						if (stat.getLastBasicType() == Statement.StatementType.IF) {
					...
					case SYNCHRONIZED, SWITCH, IF, SEQUENCE, ROOT -> {
						...
						if (statementBreakIndex <= statsize) {
							...
							if (stat.type == Statement.StatementType.IF && ((IfStatement) stat).iftype == IfStatement.IFTYPE_IF) {
								lstSuccEdges.add(stat.getSuccessorEdges(EdgeType.DIRECT_ALL).get(0));  // exactly one edge
