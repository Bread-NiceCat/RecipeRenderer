	...
	private static void mapClassMethods(ClassesProcessor.ClassNode node, Map<ClassWrapper, MethodWrapper> map) {
		...
		for (MethodWrapper method : wrapper.getMethods()) {
			...
			if ((noSynthFlag || mt.isSynthetic()) &&
				...
				if (root != null && root.getFirst().type == Statement.StatementType.TRY_CATCH) {
					CatchStatement cst = (CatchStatement) root.getFirst();
					if (cst.getStats().size() == 2 && cst.getFirst().type == Statement.StatementType.BASIC_BLOCK &&
							cst.getStats().get(1).type == Statement.StatementType.BASIC_BLOCK &&
