...
public final class LabelHelper {
	...
	private static void liftClosures(Statement stat) {
		...
		for (StatEdge edge : stat.getAllSuccessorEdges()) {
			...
			} else if (StatEdge.EdgeType.BREAK.equals(edge.getType())) {
				Statement dest = edge.getDestination();
				if (dest.type != Statement.StatementType.DUMMY_EXIT) {
	...
	public static void lowContinueLabels(Statement stat, HashSet<StatEdge> edges) {
		...
		boolean ok = (stat.type != Statement.StatementType.DO);
		if (!ok) {
			DoStatement dostat = (DoStatement) stat;
			ok = dostat.getLoopType() == DoStatement.LoopType.DO ||
					dostat.getLoopType() == DoStatement.LoopType.WHILE ||
					(dostat.getLoopType() == DoStatement.LoopType.FOR && dostat.getIncExprent() == null);
		}
	...
	public static void lowClosures(Statement stat) {
		...
		for (StatEdge edge : new ArrayList<>(stat.getLabelEdges())) {
			...
			if (edge.getType() == StatEdge.EdgeType.BREAK) {  // FIXME: ?
				for (Statement st : stat.getStats()) {
	...
	private static void setRetEdgesUnlabeled(RootStatement root) {
		...
		for (StatEdge edge : exit.getAllPredecessorEdges()) {
			...
			if (edge.getType() == StatEdge.EdgeType.FINALLY_EXIT || (lst != null && !lst.isEmpty() &&
		...
		switch (stat.type) {
			case TRY_CATCH, CATCH_ALL -> {
				for (Statement st : stat.getStats()) {
					...
					if (stat.type == Statement.StatementType.TRY_CATCH || st == stat.getFirst()) { // edges leaving a finally catch block are always explicit
						// merge the maps
						if (mapEdges1 != null) {
	...
	private static void processEdgesWithNext(Statement stat, HashMap<Statement, List<StatEdge>> mapEdges, Statement next) {
		...
		if (next == null) {
			if (mapEdges.size() == 1) {
				...
				if (lstEdges.size() > 1 && mapEdges.keySet().iterator().next().type != Statement.StatementType.DUMMY_EXIT) {
			...
			if (stat.getAllSuccessorEdges().isEmpty() && !implfound) {
				...
				for (Entry<Statement, List<StatEdge>> entr : mapEdges.entrySet()) {
					if (entr.getKey().type != Statement.StatementType.DUMMY_EXIT &&
	...
	private static LabelSets processStatementLabel(Statement stat) {
		...
		if (stat.getExprents() == null) {
			...
			boolean shieldType = (stat.type == Statement.StatementType.DO || stat.type == Statement.StatementType.SWITCH);
			if (shieldType) {
				for (StatEdge edge : stat.getLabelEdges()) {
					if (edge.explicit && ((edge.getType() == StatEdge.EdgeType.BREAK && sets.breaks.contains(edge.getSource())) ||
							(edge.getType() == StatEdge.EdgeType.CONTINUE && sets.continues.contains(edge.getSource())))) {
						edge.labeled = false;
					}
				}
			}
