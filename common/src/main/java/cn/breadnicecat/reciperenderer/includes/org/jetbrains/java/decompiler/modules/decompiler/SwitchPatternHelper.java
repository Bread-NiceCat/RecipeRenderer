...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.StatEdge.EdgeType;
...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.VBStyleCollection;
...
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.PatternHelper.*;
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.gen.VarType.*;
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.ClassNameConstants.JAVA_LANG_OBJECT;
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.ClassNameConstants.JAVA_UTIL_OBJECTS;
...
public final class SwitchPatternHelper {
	...
	static class JavacReferenceRecognizer implements SwitchHelper.SwitchRecognizer {
		...
		@Override
		public @Nullable SwitchHelper.SwitchOnCandidate recognize(@NotNull SwitchStatement statement, @NotNull InvocationExprent switchSelector) {
	...
	private static class JavacReferenceFinder {
		 ...
		 * @see SwitchStatement
		 */
		private record FullCase(@NotNull Statement statement, @NotNull List<Exprent> exprents,
		                        @NotNull List<StatEdge> edges) {
		}
			...
			for (Exprent exprent : root.firstExprents()) {
				if (exprent instanceof AssignmentExprent firstAssignment && instance.equals(firstAssignment.getLeft())) {
					...
					if (firstAssignment.getLeft() instanceof VarExprent varExprent) {
						myTempVarAssignments.add(new SwitchHelper.TempVarAssignmentItem(varExprent, root.firstStatement()));
					}
				}
			}
			for (Exprent exprent : root.firstExprents()) {
		...
		private SwitchPatternHelper.PatternContainer collectGuards(@NotNull Map<Statement, List<PatternVariableCandidate>> candidates,
			...
			for (int i = 0; i < statements.size(); i++) {
				...
				if (nameAssignment instanceof AssignmentExprent nameAssignmentExprent &&
					...
					myTempVarAssignments.add(new SwitchHelper.TempVarAssignmentItem(varExprent, ifStatement));
	 ...
	 */
	private static class SwitchOnReferenceCandidate implements SwitchHelper.SwitchOnCandidate {
		@NotNull
		private final SwitchStatement myRootSwitchStatement;
		@NotNull
		...
		private SwitchOnReferenceCandidate(@NotNull SwitchStatement rootSwitchStatement,
		                                   ...
		                                   @NotNull List<SwitchHelper.TempVarAssignmentItem> tempVarAssignments,
		...
		private static void cleanDefault(@NotNull SwitchStatement statement) {
			...
			for (int caseIndex = 0; caseIndex < caseEdges.size(); caseIndex++) {
				...
				if (defaultStatement.getExprents() != null && defaultStatement.getExprents().size() == 1) {
					...
					if (expectedFastExit instanceof ExitExprent exitExprent && exitExprent.getExitType() == ExitExprent.EXIT_THROW &&
