...
public final class InlineSingleBlockHelper {
	...
	private static void inlineBlock(SequenceStatement seq, int index) {
		...
		StatEdge edge = first.getPredecessorEdges(StatEdge.EdgeType.BREAK).get(0);
		...
		if (parent.type == Statement.StatementType.IF && ((IfStatement) parent).iftype == IfStatement.IFTYPE_IF &&
				source == parent.getFirst()) {
			IfStatement ifparent = (IfStatement) parent;
			SequenceStatement block = new SequenceStatement(lst);
			block.setAllParent();
			
			StatEdge newedge = new StatEdge(StatEdge.EdgeType.REGULAR, source, block);
			...
			for (StatEdge prededge : block.getPredecessorEdges(StatEdge.EdgeType.CONTINUE)) {
				...
				prededge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, prededge, source);
			...
			if (parent.type == Statement.StatementType.SWITCH) {
				((SwitchStatement) parent).sortEdgesAndNodes();
			}
			
			source.addSuccessor(new StatEdge(StatEdge.EdgeType.REGULAR, source, first));
		}
	}
	...
	private static boolean isInlineable(SequenceStatement seq, int index) {
		...
		List<StatEdge> lst = first.getPredecessorEdges(StatEdge.EdgeType.BREAK);
	...
	private static boolean sameCatchRanges(StatEdge edge) {
		...
		while (true) {
			...
			if (parent.type == Statement.StatementType.TRY_CATCH ||
					parent.type == Statement.StatementType.CATCH_ALL) {
				if (parent.getFirst() == from) {
					return false;
				}
			} else if (parent.type == Statement.StatementType.SYNCHRONIZED) {
	...
	private static boolean noExitLabels(Statement block, Statement sequence) {
		...
		for (StatEdge edge : block.getAllSuccessorEdges()) {
			if (edge.getType() != StatEdge.EdgeType.REGULAR && edge.getDestination().type != Statement.StatementType.DUMMY_EXIT) {
	...
	public static boolean isBreakEdgeLabeled(Statement source, Statement closure) {
		if (closure.type == Statement.StatementType.DO || closure.type == Statement.StatementType.SWITCH) {
			...
			return parent != closure &&
					(parent.type == Statement.StatementType.DO || parent.type == Statement.StatementType.SWITCH || isBreakEdgeLabeled(parent, closure));
		} else {
			return true;
		}
	}
