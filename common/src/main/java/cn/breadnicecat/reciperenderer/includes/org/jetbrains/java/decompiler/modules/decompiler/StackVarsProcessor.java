...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.sforms.*;
	...
	private static boolean iterateStatements(RootStatement root, SSAUConstructorSparseEx ssa) {
		...
		while (!stack.isEmpty()) {
			...
			if (nd.successors.size() == 1) {
				...
				if (ndsucc.type == DirectNode.DirectNodeType.TAIL && !ndsucc.exprents.isEmpty()) {
			...
			if (nd.exprents.isEmpty() &&
					(nd.type == DirectNode.DirectNodeType.INIT || nd.type == DirectNode.DirectNodeType.CONDITION || nd.type == DirectNode.DirectNodeType.INCREMENT)) {
				nd.exprents.add(null);
				
				if (nd.statement.type == Statement.StatementType.DO) {
					DoStatement loop = (DoStatement) nd.statement;
					
					if (loop.getLoopType() == DoStatement.LoopType.FOR &&
							loop.getInitExprent() == null &&
							loop.getIncExprent() == null) { // "downgrade" loop to 'while'
						loop.setLoopType(DoStatement.LoopType.WHILE);
					}
				}
			}
		}
