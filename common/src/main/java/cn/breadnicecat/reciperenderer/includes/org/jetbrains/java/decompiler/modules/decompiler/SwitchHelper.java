...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.util.ClassNameConstants;
...
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.code.CodeConstants.CLINIT_NAME;
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.main.extern.IFernflowerLogger.Severity;
import static cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.gen.VarType.VARTYPE_STRING;
	...
	static void removeTempVariableDeclarations(@NotNull List<TempVarAssignmentItem> tempVarAssignments) {
		...
		for (Statement statement : statements) {
			...
			while (parent != null) {
				...
				if (parent.getFirst() != null && parent.getFirst().type == Statement.StatementType.BASIC_BLOCK) {
					candidates = parent.getFirst().getExprents();
				} else if (parent.type == Statement.StatementType.TRY_CATCH || parent.type == Statement.StatementType.SEQUENCE) {
	...
	private abstract static class StringSwitchRecognizer implements SwitchRecognizer {
		...
		private static class JavacStringRecognizer extends StringSwitchRecognizer {
			...
			public SwitchOnStringCandidate recognize(@NotNull SwitchStatement firstSwitch,
				...
				if (switchSelector.getInstance() == null || switchSelector.getInstance().type != Exprent.EXPRENT_VAR)
					return null;
				...
				for (Statement statement : firstSwitch.getCaseStatements()) {
					...
					if (edgeDestination.type != Statement.StatementType.SWITCH) return null;
				...
				if (firstStatementInFirstSwitch.type != Statement.StatementType.BASIC_BLOCK) return null;
				Statement firstStatementInSecondSwitch = secondSwitch.getFirst();
				if (firstStatementInSecondSwitch.type != Statement.StatementType.BASIC_BLOCK) return null;
