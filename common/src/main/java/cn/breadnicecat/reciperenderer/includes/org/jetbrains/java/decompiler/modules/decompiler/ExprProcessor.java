...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.code.Instruction;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.code.InstructionSequence;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.code.cfg.BasicBlock;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.main.CancellationManager;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.main.ClassesProcessor;
...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.exps.*;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.sforms.DirectGraph;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.sforms.DirectNode;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.sforms.FlattenStatementsHelper;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.stats.*;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.typeann.TypeAnnotationWriteHelper;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.modules.decompiler.vars.VarProcessor;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.StructClass;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.StructTypePathEntry;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.attr.StructBootstrapMethodsAttribute;
...
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.consts.LinkConstant;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.consts.PooledConstant;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.consts.PrimitiveConstant;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.gen.MethodDescriptor;
import cn.breadnicecat.reciperenderer.includes.org.jetbrains.java.decompiler.struct.gen.Type;
...
public class ExprProcessor {
	...
	public void processStatement(RootStatement root, StructClass cl) {
		...
		for (List<FlattenStatementsHelper.FinallyPathWrapper> wrappers : dgraph.mapShortRangeFinallyPaths.values()) {
			for (FlattenStatementsHelper.FinallyPathWrapper wrapper : wrappers) {
				setFinallyShortRangeEntryPoints.add(wrapper.entry);
			}
		}
		
		Set<String> setFinallyLongRangeEntryPaths = new HashSet<>();
		for (List<FlattenStatementsHelper.FinallyPathWrapper> wrappers : dgraph.mapLongRangeFinallyPaths.values()) {
			for (FlattenStatementsHelper.FinallyPathWrapper wrapper : wrappers) {
	...
	private static int nextMeaningfulOffset(BasicBlock block, int index) {
		...
		while (++index < seq.length()) {
			switch (seq.getInstr(index).opcode) {
				case CodeConstants.opc_nop, CodeConstants.opc_istore, CodeConstants.opc_lstore,
				     CodeConstants.opc_fstore, CodeConstants.opc_dstore, CodeConstants.opc_astore -> {
					continue;
				}
			}
	...
	public static TextBuffer jmpWrapper(Statement stat, int indent, boolean semicolon, BytecodeMappingTracer tracer) {
		...
		if (successorEdges.size() == 1) {
			...
			if (edge.getType() != StatEdge.EdgeType.REGULAR && edge.explicit && edge.getDestination().type != Statement.StatementType.DUMMY_EXIT) {
