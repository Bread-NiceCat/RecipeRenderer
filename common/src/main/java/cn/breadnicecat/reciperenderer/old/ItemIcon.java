...
public class ItemIcon {
	...
	public ItemIcon(int size, ItemStack item) {
		this(size, item, null);
	}
	
	public ItemIcon(int size, ItemStack item, LivingEntity holder) {
		this.size = size;
		this.item = item;
		this.holder = holder;
		//开始
		...
		target.bindRead();
		try (var ni = new NativeImage(target.width, target.height, false)) {
			ni.downloadTexture(0, false);
			target.destroyBuffers();
			ni.flipY();
			image = ni.asByteArray();
